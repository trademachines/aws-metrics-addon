...
const getServiceByDeploymentId = (ecs, eventInfo, serviceDeploymentId, cb) => {
    ...
    async.doUntil(
        ...
        (err) => {
            ...
            if (!service) {
                return cb(new Error(`Cant't find service with deployment id ${serviceDeploymentId} from ${JSON.stringify(eventInfo)}`));
            }
    ...
    async.auto(
        {
            ...
            metrics: [ 'service', 'tasks', (results, cb) => extractMetricsWithDebug(event, info, results.service, results.tasks, cb) ],
        },
